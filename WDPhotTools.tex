...
also available with this toolkit in Section~\textsection6, and we summarise this
work in Section~\textsection7.
...
\begin{enumerate}
    ...
    \textcolor{red}{(see also Section~\textsection4.3 and
...
within N-sigmas uncertainty limit (user-defined). \textcolor{red}{The
uncertainties are not computed when using the `minimize()` moethod, while
...
At lower temperatures, the atmospheric \textcolor{red}{compositions} have
much stronger effects. The average mass of a population drops when fitted
with pure H model, pure He, and then to mixed H/He models. The
...
\begin{multline}
    \label{eq:chi2}
    \chi^{2} = \left[\dfrac{2.5}{\ln(10)}\right]^2 \times \sum_{i}\left\{ \dfrac{1}{\sigma_{M, i}^2} \left[ \left(\dfrac{10}{D}\right)^{2} \times \\ 10^{\frac{m_{i} - M_{\mathrm{model}, i} - A_{i}(D)}{2.5}} - 1 \right]^2 \right\}
\end{multline}
...
of Table 6 in SF11. Our values are making use of WD spectra instead of using a single spectrum of an MS star at 7000\,K in SF11, which is intended to be a good set of average values for general use.}
The synthetic
...
\begin{table*}
    ...
    \begin{tabular}{cccccccc}
        ...
        \multicolumn{8}{c}{Montreal smooth grid} \\\hline
        \citet[][B20]{2020ApJ...901...93B} & \checkmark &  \checkmark  & \checkmark &    CO &     \textcolor{black}{H/He} &            $0.2-1.3$             & -- \\
        &&&&&&&\\

        \multicolumn{8}{c}{LPCODE smooth grid~(lpcode\_da\_22)} \\\hline
